    ...
    CollisionBody2D const& b)
{
    static auto is_collide_bvb = [] (auto const& a, auto const& b) -> bool {
        ...
        bool collide_if = (topleft_a.x < bottomright_b.x and bottomright_a.x > topleft_b.x)
            and (topleft_a.y > bottomright_b.y and bottomright_a.y < topleft_b.y);
...
float DistanceDetectorStrategy2DUsePythagorean::distance_between(CollisionBody2D const& a, CollisionBody2D const& b)
{
    float diff_x = a.m_position.x - b.m_position.x;
    float diff_y = a.m_position.y - b.m_position.y;

    return std::sqrt(std::pow(diff_x, 2) + std::pow(diff_y, 2));
}
